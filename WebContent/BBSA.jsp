	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					   <%
                            ...
                            ArrayList<Bbs> Alist = BbsDAO.getListA(pageNumber);
                            for (int i = 0; i < Alist.size(); i++){                    
                        %>
                        <tr>
                        <td class = "alpha" align="left" width="40%"><a 
                        href="view.jsp?bbsID=<%=Alist.get(i).getBbsID()%>"><%=Alist.get(i).getBbsTitle().replaceAll(" ", "&nbsp;")%></a></td>
				        <td width="30%"><%=Alist.get(i).getUserID()%></td>
				        <td width="30%"><%=Alist.get(i).getBbsDate().substring(0, 11)%></td>
				        </tr>
				        <%
                            }
				        %>
			...
			<%
					if (pageNumber != 1) {
				%>
			<a href="BBSA.jsp?pageNumber=<%=pageNumber - 1%>"
			...
			<button class="btn btn-primary pull-right"
				onclick="if(alert('관리자 계정으로는 사용 불가능한 기능입니다.')); location.href='BBSA.jsp';"
